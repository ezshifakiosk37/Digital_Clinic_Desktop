...
const CALIBRATION_FACTOR = 1.04; // Adjust only this if needed

interface HeightCameraModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (heightFeetDotInches: string) => void;
}
...
const HeightCameraModal: React.FC<HeightCameraModalProps> = ({ isOpen, onClose, onConfirm }) => {
    ...
    const [barY, setBarY] = useState<number>(120);
    ...
    const startCamera = () => setStep('camera');

    const stopCamera = useCallback(() => {
        // Webcam component handles cleanup
    }, []);
    ...
    const onImgLoad = useCallback(() => {
        ...
        setImgDisplayH(h);
        setBarY(h * 0.22); // start a bit below top
    }, []);

    // Drag handlers
    const clamp = (y: number, max: number) => Math.max(10, Math.min(max - 10, y));

    const handleDrag = (clientY: number) => {
        if (!imgRef.current) return;
        const rect = imgRef.current.getBoundingClientRect();
        setBarY(clamp(clientY - rect.top, rect.height));
    };
    ...
    const calculateHeight = useCallback(() => {
        ...
        let heightInches = Math.round(
            (pixelsFromHeadToFloor / displayH) * VISIBLE_HEIGHT_INCHES * CALIBRATION_FACTOR
        );

        // Round to nearest integer inches
        ...
        <div className="fixed inset-0 z-[9999] bg-white flex flex-col">
            ...
            {step === 'intro' && (
                <div className="flex-1 flex flex-col items-center justify-center p-6 bg-white">
                    <div className="w-20 h-20 bg-[#0297d6]/10 rounded-3xl flex items-center justify-center mb-6">
                    ...
                    <h2 className="text-3xl font-bold text-slate-800 mb-2">Measure Height</h2>
                    <p className="text-slate-600 text-center max-w-xs mb-8">
                        Stand <span className="font-semibold text-[#0297d6]">6 feet away</span> from the camera. Full body must be visible.
                    </p>

                    <div className="grid grid-cols-2 gap-4 w-full max-w-xs mb-8">
                        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-center">
                            <div className="text-[#0297d6] text-3xl font-bold">4 ft</div>
                            <div className="text-xs text-slate-500 mt-1">Camera Height</div>
                        </div>
                        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-center">
                            <div className="text-[#0297d6] text-3xl font-bold">6 ft</div>
                            <div className="text-xs text-slate-500 mt-1">Stand Here</div>
                        </div>
                    </div>

                    <button
                        onClick={startCamera}
                        className="w-full max-w-xs py-4 rounded-2xl bg-[#0297d6] hover:bg-[#0286c2] text-white font-bold text-lg transition-all"
                    >
                        Open Camera
                    </button>
                </div>
                ...
                <div className="relative flex-1 bg-black">
                    ...
                    <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
                        <button
                            ...
                            className="bg-black/50 text-white px-4 py-2 rounded-xl text-sm font-medium"
                        >
                            ← Back
                        </button>
                        <div className="text-white text-sm font-medium bg-black/50 px-4 py-1 rounded-xl">
                            Stand 6 ft away • Floor visible
                        </div>
                    </div>
                    ...
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
                        ...
                        >
                            <div className="w-16 h-16 bg-[#0297d6] rounded-full" />
                        </button>
                    </div>
                </div>
            )}
            ...
            {step === 'adjust' && capturedImage && (
                <div className="flex-1 flex flex-col bg-white">
                    <div className="p-4 border-b flex items-center justify-between bg-white">
                        <button onClick={() => setStep('camera')} className="text-[#0297d6] font-medium">← Retake</button>
                        <p className="font-semibold text-slate-700">Drag red line to top of head</p>
                        <div className="w-10" />
                    </div>

                    <div className="flex-1 relative overflow-hidden bg-slate-100" style={{ touchAction: 'none' }}>
                        <img
                            ref={imgRef}
                            src={capturedImage}
                            alt="Captured"
                        ...
                        {/* Floor line */}
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#0297d6]" />

                        {/* Draggable Red Bar */}
                        <div
                            className="absolute left-0 right-0 z-20 cursor-row-resize"
                            style={{ top: barY - 20 }}
                            onMouseDown={(e) => { setIsDragging(true); handleDrag(e.clientY); }}
                            onTouchStart={(e) => { setIsDragging(true); handleDrag(e.touches[0].clientY); }}
                            onMouseMove={(e) => isDragging && handleDrag(e.clientY)}
                            onTouchMove={(e) => isDragging && handleDrag(e.touches[0].clientY)}
                            onMouseUp={() => setIsDragging(false)}
                            onTouchEnd={() => setIsDragging(false)}
                        >
                            <div className="h-[3px] bg-red-500 w-full shadow-[0_0_10px_#ef4444]" />
                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded">
                                HEAD
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-t bg-white">
                        <button
                            ...
                            className="w-full py-4 bg-[#0297d6] hover:bg-[#0286c2] text-white font-bold rounded-2xl text-lg"
                        >
                            Calculate Height
                        </button>
                    </div>
            ...
            {step === 'confirm' && (
                <div className="flex-1 flex flex-col bg-white">
                    <div className="p-4 border-b flex justify-between items-center">
                        <button onClick={() => setStep('adjust')} className="text-[#0297d6]">← Adjust</button>
                        <p className="font-semibold">Height Result</p>
                        <button onClick={onClose} className="text-slate-400">✕</button>
                    </div>

                    <div className="flex-1 flex items-center justify-center bg-slate-50 relative">
                        {capturedImage && <img src={capturedImage} className="max-h-full opacity-70" alt="" />}
                        <div className="absolute text-center">
                            <div className="text-7xl font-black text-[#0297d6] mb-2">
                    ...
                    <div className="p-4 border-t bg-white flex gap-3">
                        ...
                        <button
                            onClick={() => {
                                onConfirm(calculatedHeight);
                                onClose();
                            }}
                            className="flex-1 py-4 bg-[#0297d6] hover:bg-[#0286c2] text-white font-bold rounded-2xl"
                        >
                            Use This Height ✓
                        </button>
                    </div>
